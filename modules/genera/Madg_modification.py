...
def Madg_modification(Event,  # number of event simulate
    ...
    file_clear(DirMadgraph + "/models/MSSMD_UFO/", "tree", info=info)  # Borrar el archivo con contenido
    ...
    for j in range(ite):
        if not file_exists(DirMadgraph + "/MSSMD", mode="empty", info=info, local=True):
            file_copy(Dir_Source + "/proc_card.dat", DirMadgraph, "ft", info=info)  # Copy proc_card.dat
            execute("./bin/mg5_aMC proc_card.dat >/dev/null 2>&1", info=info, position=DirMadgraph)
