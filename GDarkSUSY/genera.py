...
#    print(len(MassPhoton))
    for num in MassPhoton:
        ...
        os.system("mkdir " + DirProg + "/output/DarkSUSY_MassPhoton_" + str(num) + "_Default")  # CREAR CARPETA
        shutil.copytree(DirMadgraph + "/MSSMD/Event",
                        DirProg + "/output/DarkSUSY_MassPhoton_" + str(num) + "_Default")  # Copiar la info
...
if __name__ == '__main__':
    ...
    parser.add_argument("-mphoton", help="Mass of the Photon")
    ...
    if args.mphoton == None:
        args.mphoton=[.250, .500, 1.000, 2.000, 3.000, 5.000, 10.000]
        print(" :: Using default Photon Mass : 0.1, .2, .3, .5, 1, 2, 3, 5, 10")
    else:
        print(" :: Using Photon Mass : " + str(args.mphoton))
    #    mphoton=[]
    #    mphoton = float(args.mphoton)
#            args.mphoton = bytearray()
    ...
    if os.path.exists(args.dirmad):
        ...
        # eventos = 1000, MassPhoton = None, fileName = "DarkSUSY", dirmad = "source/MG5_aMC_v2_6_7"

        # print(str(os.sys.argv[1:]))
        # print(__doc__)
        # main(os.sys.argv)
        # for i in range(1, len(os.sys.argv)):
        #   if
        # print ([i*i for i, e in enumerate(os.sys.argv, 1) if i != os.sys.argv[0]])
        # for i in range(1, len(os.sys.argv)):
        #    print(i)
        #    print(str(os.sys.argv[i]))
